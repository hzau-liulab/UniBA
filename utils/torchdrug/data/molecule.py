...
# from utils.torchdrug import utils
# from utils.torchdrug.data import constant, Graph, PackedGraph
# from utils.torchdrug.core import Registry as R
# from utils.torchdrug.data.rdkit import draw
# from utils.torchdrug.utils import pretty
...
class Molecule(Graph):
    ...
    # print(mol, atom_feature, bond_feature, mol_feature, with_hydrogen, kekulize)
    # <rdkit.Chem.rdchem.Mol object at 0x00000290B577C2E0> default, default, None, False, False
    def from_molecule(cls, mol, atom_feature="default", bond_feature="default", mol_feature=None,
        ...
        # note this only affects the relation in ``edge_list``
        # in cg scale, the aromatic bonds could be included in one bead, such that the kekulize does not need to be explicitly used
        if kekulize:
            Chem.Kekulize(mol)
        ...
        if mol.GetNumConformers() > 0:
            ...
            node_position = None
        # print(atoms, atom_feature, bond_feature, mol_feature, mol.GetNumConformers(), node_position)
        # ['default'] ['default'] [] 1
        # tensor([[-15.5290,  -2.3650,  31.1630],
        #         [-16.9420,  -2.1410,  30.8100],
        #         [-17.7660,  -2.2580,  32.0770],
        #         ...,
        #         [-27.7850,   6.6480,   7.8950],
        #         [-26.5710,   5.1630,   9.3900],
        #         [-22.1720,  10.6470,  12.3300]], dtype=torch.float64)

        for atom in atoms:
            atom_type.append(atom.GetAtomicNum()) # Returns the atomic number
            formal_charge.append(atom.GetFormalCharge()) # GetFormalCharge
            explicit_hs.append(atom.GetNumExplicitHs()) # GetNumExplicitHs
            ...
            for name in atom_feature:
                ...
                # print(func, "features.atom.%s" % name)
                # <function atom_default at 0x0000020EF8709820> features.atom.default
                feature += func(atom)
        ...
        if len(atom_feature) > 0:
            ...
            _atom_feature = None
        # print(_atom_feature.size()) # torch.Size([6562, 67])
        ...
        # print(edge_list, len(edge_list), [dummy_bond])
        # 13484 [<rdkit.Chem.rdchem.Bond object at 0x0000028F93E48970>]
        ...
        # print(num_relation) # 4

        # print(len(edge_list), edge_list) # 13482, [6552, 6550, 0], [6550, 6552, 0]
        # print(atom_type, atom_type.size()) # tensor([7, 6, 6,  ..., 6, 6, 8]) torch.Size([6562]), the atomic number
        # print(bond_type, bond_type.size()) # tensor([0, 0, 0,  ..., 3, 0, 0]) torch.Size([13482]), bond2id = {"SINGLE": 0, "DOUBLE": 1, "TRIPLE": 2, "AROMATIC": 3}
        # print(atom_map.sum(), mol.GetNumAtoms()) # tensor(0) -> invalid, 6562
    ...
    @classmethod
    # @utils.deprecated_alias(node_feature="atom_feature", edge_feature="bond_feature", graph_feature="mol_feature")
    @utils.deprecated_alias(node_feature="atom_feature", edge_feature="bond_feature")
