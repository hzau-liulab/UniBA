...
class Graph(core._MetaContainer):
    ...
    # input information: 1: edge_list 2: {'num_node': 439, 'num_relation': 5}
    ...
    #     max_edge = int(edge_list[:, :2].max())
    #     if max_edge >= num_node:
    #         raise ValueError(
    #             f"edge index {max_edge} exceeds num_node {num_node}"
    #         )
    #     return torch.as_tensor(num_node, device=edge_list.device)
    ...
    def pack(cls, graphs):
        ...
        # cls.packed_type, cls: <class 'torchdrug.data.molecule.PackedMolecule'> <class 'cg_steps.cg_protein.CG22_Protein'>
        # ** actually, below way of initialization can satisfy the requirement of creating an effective CG Protein Class **
        # ** cls.packed_type is defined below: Graph.packed_type = PackedGraph (is actually the PackedGraph class) **
    ...
    def data_mask(self, node_index=None, edge_index=None, graph_index=None, include=None, exclude=None):
        ...
        for k, v in data_dict.items():
            for type in meta_dict[k]:
                # print(k, meta_dict[k], type)
                # bead2residue {'node', 'residue reference'} node
                # bead2residue {'node', 'residue reference'} residue reference
    ...
    def node_mask(self, index, compact=False):
        ...
        edge_list, edge_index, num_node = self.update_edge_and_node(index, compact)

        valid_edge_list = edge_list[edge_index]
        valid_nodes = torch.unique(valid_edge_list[:, :2])

        if valid_nodes.max().item()+1 != num_node:
            index = valid_nodes
            edge_list, edge_index, num_node = self.update_edge_and_node(index, compact)
    ...
    def update_edge_and_node(self, index, compact):
        mapping = -torch.ones(self.num_node, dtype=torch.long, device=self.device)
        if compact:
            mapping[index] = torch.arange(len(index), device=self.device)
            num_node = len(index)
        ...
        return edge_list, edge_index, num_node
    ...
    def clone(self):
        ...
        Clone this graph.
        """
        # print(type(self)) # <class 'cg_steps.cg_protein.CG22_Protein'>
...
class PackedGraph(Graph):
    ...
    def __init__(self, edge_list=None, edge_weight=None, num_nodes=None, num_edges=None, num_relation=None,
        ...
        # print(num_nodes) # [tensor(237), tensor(120), tensor(231), tensor(120)]
        ...
        # print(self.num_nodes, self.num_edges, self.num_cum_nodes, self.num_cum_edges)
        # tensor([237, 120, 231, 120]), tensor([544, 244, 500, 244]), tensor([237, 357, 588, 708]), tensor([544, 788, 1288, 1532])
    ...
    def _get_offsets(self, num_nodes=None, num_edges=None, num_cum_nodes=None, num_cum_edges=None):
        ...
        # print(num_cum_nodes - num_nodes, num_cum_nodes) # tensor([0, 237, 357, 588]) tensor([237, 357, 588, 708])
        # print(num_edges) # tensor([544, 244, 500, 244])
        # print((num_cum_nodes - num_nodes).repeat_interleave(num_edges)) # tensor([0,   0,   0,  ..., 588, 588, 588]) torch.Size([1532])
