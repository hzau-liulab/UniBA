...
class GeometricRelationalGraphConv(RelationalGraphConv):
    ...
    def message_and_aggregate(self, graph, input):
        ...
        adjacency = torch.sparse_coo_tensor(torch.stack([node_in, node_out]), graph.edge_weight,
                                            (graph.num_node, graph.num_node * graph.num_relation)).to(self.device)
        update = torch.sparse.mm(adjacency.t(), input)  # [num_node * num_relation, input_dim]
        if self.edge_linear:
            edge_input = graph.edge_feature.float().to(self.device)
            edge_input = self.edge_linear(edge_input)
            edge_weight = graph.edge_weight.unsqueeze(-1).to(self.device)
            edge_update = scatter_add(edge_input * edge_weight, node_out.to(self.device), dim=0,
                                      dim_size=graph.num_node * graph.num_relation)  # [num_node * num_relation, input_dim]
            update += edge_update.to(self.device)

        return update.view(graph.num_node, self.num_relation * self.input_dim)


class AttentiveGeometricRelationalGraphConv(nn.Module):
    def __init__(self, input_dim, output_dim, num_relation, edge_input_dim=None, batch_norm=False, activation='relu'):
        super().__init__()
        self.num_relation = num_relation
        self.input_dim = input_dim
        ...
        self.linear_relation = nn.Linear(num_relation * input_dim, output_dim)
        ...
        self.attn_fc = nn.Linear(2 * output_dim, 1)
    ...
    def forward(self, graph, node_input):
        ...
        h = self.linear_node(node_input)  # (N, d)

        node_in, node_out, rel = graph.edge_list.t()
        src = h[node_in]
        dst = h[node_out]

        attn_input = torch.cat([src, dst], dim=-1)
        e = self.attn_fc(attn_input).squeeze(-1)  # (E,)
        e = F.leaky_relu(e)
        alpha = scatter_add(e.exp(), node_out, dim=0, dim_size=h.size(0))
        alpha = e.exp() / (alpha[node_out] + 1e-6)  # normalize over neighbors

        msg = src * alpha.unsqueeze(-1)

        if self.edge_linear:
            edge_feat = graph.edge_feature.float().to(msg.device)
            edge_feat = self.edge_linear(edge_feat)
            msg += edge_feat

        update = scatter_add(msg, node_out, dim=0, dim_size=h.size(0))  # 聚合消息
