...
class Engine(core.Configurable):
    ...
    def __init__(self, task, train_set, valid_set, test_set, optimizer, scheduler=None, gpus=1, batch_size=1,
        ...
        self.model = task
        self.train_set = train_set # used in solver.train (independent of solver.evaluate)
        self.valid_set = valid_set # None, could be used in solver.evaluate
        self.test_set = test_set # None, could be used in solver.evaluate
        self.optimizer = optimizer
        self.scheduler = scheduler
        # print(self.train_set, self.valid_set, self.test_set)
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        for epoch in self.meter(num_epoch):
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                ...
                # we do not need to change the overall outer training loop scheme due to the property of diffusion model
                loss, metric = model(batch)
                # loss: tensor(4.1210, device='cuda:0', grad_fn=<AddBackward0>)
                # metric: {'structure denoising loss': tensor(5.2460, device='cuda:0', grad_fn=<AddBackward0>),
