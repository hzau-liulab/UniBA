...
class AdvSpatialEdge(nn.Module, core.Configurable):
    ...
    def __init__(self, radius=5, min_distance=5, max_distance=None, max_num_neighbors=32,
                 across_res_mask=True, cg_edge_enhance=False, cg_edge_reduction=True):
    ...
    def forward(self, graph, pr_seed):
        ...
        def sample_radius_from_energy(node_energy, pr_seed, alpha=2.0, beta=1.5, r_min=1.0, r_max=5.0):
            if pr_seed is not None:
                torch.manual_seed(pr_seed)

            alpha_param = alpha + beta * node_energy
            beta_param = alpha + beta * (1 - node_energy)

            beta_dist = Beta(alpha_param, beta_param)
            r_sample = beta_dist.sample()  # (N,)

            # 映射到目标半径范围
            radius = r_min + r_sample * (r_max - r_min)
            # print(alpha_param, beta_param, radius)
            return radius  # (N,)

        def energy_based_radius_graph(node_pos, node_radius, node2graph, max_num_neighbors=32):
            """
            Build edge index using node-wise radius based on energy
            node_pos: (N, 3)
            node_radius: (N,) radius per node
            node2graph: (N,) graph IDs
            Return: (E, 2) edge index
            """
            import torch_cluster

            edge_list = []
            for graph_id in torch.unique(node2graph):
                mask = (node2graph == graph_id)
                pos = node_pos[mask]  # (n, 3)
                radius = node_radius[mask]  # (n,)
                local_to_global = torch.nonzero(mask, as_tuple=False).squeeze(-1)

                for i in range(pos.size(0)):
                    center = pos[i].unsqueeze(0)  # (1, 3)
                    dist = torch.norm(pos - center, dim=1)  # (n,)
                    neighbor_idx = torch.nonzero(dist <= radius[i], as_tuple=False).squeeze(-1)
                    neighbor_idx = neighbor_idx[neighbor_idx != i]  # exclude self

                    if max_num_neighbors is not None and len(neighbor_idx) > max_num_neighbors:
                        neighbor_idx = neighbor_idx[:max_num_neighbors]

                    src = local_to_global[i].repeat(len(neighbor_idx))
                    dst = local_to_global[neighbor_idx]
                    edge_list.append(torch.stack([src, dst], dim=0))

            edge_index = torch.cat(edge_list, dim=1) if edge_list else torch.empty((2, 0), dtype=torch.long)
            return edge_index.t()  # shape (E, 2)
        ...
        # 将两种能量融合为一个 scalar，方式可以是 mean, sum, 或可学习加权
        # node_energy = graph.node_energy_feature.mean(dim=1)  # shape: (N,)
        # node_energy = 0.3 * graph.node_energy_feature[:, 0] + 0.7 * graph.node_energy_feature[:, 1]
        #
        # node_radius = sample_radius_from_energy(node_energy, pr_seed)
        # edge_list = energy_based_radius_graph(graph.node_position, node_radius, graph.node2graph)

        edge_spatial = radius_graph(graph.node_position, r=self.radius, batch=graph.node2graph,
                                    max_num_neighbors=self.max_num_neighbors).t()

        relation = torch.zeros(len(edge_spatial), 1, dtype=torch.long, device=graph.device)
        edge_spatial = torch.cat([edge_spatial, relation], dim=-1)
        num_relation = 1

        if self.min_distance > 0 and len(edge_spatial) > 0:
            node_in, node_out = edge_spatial.t()[:2]
            ...
            edge_spatial = edge_spatial[~mask]

        if self.max_distance and len(edge_spatial) > 0:
            node_in, node_out = edge_spatial.t()[:2]
        ...
        if self.across_res_mask and len(edge_spatial) > 0:
            node_in, node_out = edge_spatial.t()[:2]  # node_in: source_node, node_out: target_node
            # the bead2residue id should be the same for both end nodes in the same residue
            across_res_mask = (graph.bead2residue[node_in] - graph.bead2residue[
                node_out]).abs() > 0  # True for across residue edges
            edge_spatial[:, 2][across_res_mask] = 1  # intra residue edges: 0, across/inter residue edges: 1
            num_relation += 1
        ...
        if self.cg_edge_enhance:
            ...
            edge_list = torch.cat([edge_spatial, edge_list_enhance], dim=0)
